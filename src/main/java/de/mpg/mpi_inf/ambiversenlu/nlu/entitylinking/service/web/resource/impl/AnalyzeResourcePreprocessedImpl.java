...
public class AnalyzeResourcePreprocessedImpl implements AnalyzeResourcePreprocessed {
  ...
  static org.slf4j.Logger defaultLogger = org.slf4j.LoggerFactory.getLogger(AnalyzeResourcePreprocessedImpl.class);
  ...
  @Override public PostAnalyzeResponse postAnalyze(AnalyzeInput input) {
    ...
    try {
      ...
      AnalyzeOutput elo = OutputUtilsPreprocessed.generateAnalyzeOutputfromProcessedDocument(dp.process(doc));
...
//      e.printStackTrace();
  ...
  public static PipelineType getSaliencePipelineByNerConfig() {
    if (EntityLinkingConfig.get(EntityLinkingConfig.WEBSERVICE_NER).equals("knowner")) {
      return PipelineType.PREPROCESSED_ENTITY_SALIENCE;
    } else {
      return PipelineType.PREPROCESSED_ENTITY_SALIENCE_STANFORD;
    }
  }

  public static PipelineType getConceptSaliencePipelineByNerConfig() {
    if (EntityLinkingConfig.get(EntityLinkingConfig.WEBSERVICE_NER).equals("knowner")) {
      return PipelineType.PREPROCESSED_ENTITY_CONCEPT_SALIENCE;
    } else {
      return PipelineType.PREPROCESSED_ENTITY_CONCEPT_SALIENCE_STANFORD;
    }
  }
