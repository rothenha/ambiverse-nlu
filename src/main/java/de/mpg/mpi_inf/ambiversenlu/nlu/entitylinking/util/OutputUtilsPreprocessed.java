...
public class OutputUtilsPreprocessed {
  ...
  public static AnalyzeOutput generateAnalyzeOutputfromProcessedDocument(ProcessedDocument pd) throws EntityLinkingDataAccessException {
    ...
    Tokens tokens = pd.getTokens();
    Map<Integer, Integer> mapBeginOffsetToTokenIndex = new HashMap<>();
    Map<Integer, Integer> mapEndOffsetToTokenIndex = new HashMap<>();
    
    int nTokenIndex = 0;
    for (Token token : tokens) {
      mapBeginOffsetToTokenIndex.put(token.getBeginIndex(), nTokenIndex);
      mapEndOffsetToTokenIndex.put(token.getEndIndex(), nTokenIndex);
      nTokenIndex++;
    }
    ...
    for (ResultMention rm : pd.getEntityAnnotations()) {
      ...
      int nBeginCharacterOffset = rm.getCharacterOffset();
      int nFirstTokenIndex = mapBeginOffsetToTokenIndex.get(nBeginCharacterOffset);
      match.setCharOffset(nFirstTokenIndex);
      int nCharLength = rm.getCharacterLength();
      int nEndCharOffset = nBeginCharacterOffset + nCharLength;
      int nTokenLength = mapEndOffsetToTokenIndex.get(nEndCharOffset) - nFirstTokenIndex + 1;
      match.setCharLength(nTokenLength);
